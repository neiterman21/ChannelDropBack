...
def build_model_torch(args):
    ...
    pretrain_weights = {
        'ViT-B_16' : 'ViT-pytorch/ViT-B_16.npz',
        'ViT-L_16' : 'pretrain/ViT-L_16.npz',
        'ViT-H_16' : 'pretrain/ViT-H_16.npz'
    }

    model = VisionTransformer(config, (imagenet_res,imagenet_res), zero_head=True, num_classes=no_of_class)
    model.load_from(np.load(pretrain_weights[args.model]))
    return model
...
def train_model(model, device, train_loader, optimizer, criterion):
    ...
    for batch_idx, (data, target) in enumerate(train_loader, start=0):
        ...
        scores, predictions = torch.max(output.view(-1, 10).data, 1)
...
def test_model(model, device, test_loader, criterion):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            data, target = data.to(device), target.to(device)
            output  = model(data)
            test_loss += criterion(output.view(-1, 10), target).item()
...
def main(args):
    ...
    # torch.manual_seed(2836748912)
    # np.random.seed(182673)
      
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    

    dropback_net = build_model_torch(args)
...
def get_parser():
    ...
    parser.add_argument('--epochs', default=85, type=int, help='epochs')
    ...
    parser.add_argument('--image_res', default=224, type=int, help='image resolution')
...
if __name__ == '__main__':
    args = get_parser()
    args=Config(dropback_rate_final=args.dropback_rate_final, dropback_rate_init=0.03, dropback_rate_channels=args.dropback_rate_channels, skip_init_layers=3,
                    lr_dropback=args.lr, gamma_dropback=args.lr_gamma,batch_size=args.batch_size,
                    epochs=args.epochs,dataset=args.dataset,model=args.model, imagenet_path=args.imagenet_path)
    main(args)
